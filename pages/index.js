...
export default function Home() {
  ...
  // LOG IN SYSTEM 
  // using react state (hook) to change display of state depending on logged in or not 
  ...
  const connect = async function () {
    ...
    // useEffect + conditional for setting logged in if there is at least one account

    useEffect(function () {
      // if accounts has some value, display logged in username 
      if (accounts != null) {
        setIsLoggedIn(true)
      } else {
        setIsLoggedIn(false)
    ...
    // use await and async to delay loading 
    useEffect(async function () { 
      let a = await window.ethereum.request({ method: "eth_accounts" })
      setAccounts(a)

      // make the disconnect and connect thing work 
    ...
    // if finished loading using props in component
    // use map to loop over components 
    if (!isLoading) {
